 ...
 * 특이한 점은 에지에 해당하는 이동하는 시간이 양수가 아닌 0 또는 음삭가 가능하다는 것이다.
 * 이렇게 시작점에서 다른 노드와 관련된 최단 거리를 구하는데, 에지가 음수가 가능할 때는 벨만-포드 알고리즘을 사용하면 된다.
 * 
 * 1. 에지 리스트에 에지 데이터를 저장한 후 거리 배열을 다음과 같이 초기화합다.
 ...
 * 		3) 음 수 사이클 유무를 알기 위해 모든 에지와 관해 다시 한 번 1)을 수행한다.
...
public class 타임머신으로_빨리_가기_벨만포드 {
	...
	public static void main(String[] args) throws IOException  {
		...
		// 벨만-포드 알고리즘 수행하기
		distance[1] = 0;
		for(int i = 1; i < N; i++) {		// 노드보다 1개 적은 수만큼 반복하기
